const token = "NzEwOTkyODQ2MjIzNTczMDQ0.Xr8h-A.kMB4fyOla4b7BRuz8H4-Q-hND5E";
	...
	if (magicianSwapped && werewolfKilledTonight >= werewolfCount && prophetChecked) {
		...
		if (day !== 3) {
			...
			if (!flag) {
				...
				embed = embed.setDescription(listOfPlayers);
				playChannel.send(embed);
				const randomNum = Math.floor(Math.random() * maxPlayers);
...
function giveCharacters(msg, players) {
	...
	for (let i=0; i<maxPlayers; i++) {
		...
		identities.push(characters[randomNum]);
		remainingCharacters.splice(remainingCharacters.indexOf(characters[randomNum]));

		const thePlayer = players[i];
		thePlayer.send(`Your character is ${characters[randomNum]}`);
	}
}
